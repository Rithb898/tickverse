...
export const createBooking = async (req, res) => {
  try {
    ...
    try {
      ...
      for (const seatId of seatIds) {
        await conn.query('INSERT INTO bookings (user_id, seat_id, showtime_id) VALUES ($1, $2, $3)', [userId, seatId, showtimeId]);
        await conn.query('UPDATE seats SET isbooked = 1 WHERE id = $1', [seatId]);
      }
      
      await conn.query('COMMIT');
      res.json({ success: true, bookedSeats: seatIds, message: 'Booking successful' });
    } catch (error) {
      await conn.query('ROLLBACK');
      throw error;
    } finally {
...
export const cancelBooking = async (req, res) => {
  try {
    const bookingResult = await pool.query('SELECT * FROM bookings WHERE id = $1 AND user_id = $2', [req.params.id, req.userId]);
    ...
    await pool.query('UPDATE seats SET isbooked = 0 WHERE id = $1', [booking.seat_id]);
